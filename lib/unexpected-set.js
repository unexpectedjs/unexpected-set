...
module.exports = {
  ...
  installInto: function unexpectedSet(expect) {
    ...
    expect.addAssertion('<Set> [not] to contain <any>', (expect, subject, value) => {
      expect.withError(
        () => {
    ...
    expect.addAssertion('<Set> to [exhaustively] satisfy <Set>', (expect, subject, value) => {
      const valueElements = [];
    ...
    expect.addAssertion(
      ...
      (expect, subject, valueElements) => {
        ...
        const promiseByValueIndexAndSubjectIndex = valueElements.map((valueElement, valueIndex) => subjectElements.map((subjectElement, subjectIndex) => {
        ...
        return expect.promise
          ...
          .then(() => {
            if (
              !promiseByValueIndexAndSubjectIndex.every(row => row.some(promise => promise.isFulfilled())) ||
              (expect.flags.exhaustively &&
                !subjectElements.every((subjectElement, i) => promiseByValueIndexAndSubjectIndex.some(row => row[i].isFulfilled())))
            ) {
              expect.fail({
                diff(output, diff, inspect, equal) {
                  output.inline = true;
                      ...
                      .block(function() {
                        ...
                        if (
                          ...
                          !promiseBySubjectIndexAndValueIndex[
                            subjectIndex
                          ].some((promise, valueIndex) => promiseBySubjectIndexAndValueIndex[
                            subjectIndex
