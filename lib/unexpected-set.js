...
module.exports = {
    ...
    installInto: function unexpectedSet(expect) {
            ...
            diff: function (actual, expected, output, diff, inspect, equal) {
                ...
                expected.forEach(function (expectedElement) {
                    if (!actual.has(expectedElement)) {
                        output.nl(index > 0 ? 1 : 0).i().annotationBlock(function () {
        ...
        expect.addAssertion('<Set> [not] to contain <any>', function (expect, subject, value) {
        ...
        expect.addAssertion([
            ...
            subject.forEach(function (subjectElement) {
                if (typeof nextArg === 'string') {
                    expected.push(expect.it(function (s) {
            ...
            return expect.withError(function () {
                return expect(subjectElements, 'to [exhaustively] satisfy', expected);
            }, function (err) {
                expect.fail({
            ...
            return expect.promise.settle(promiseByValueIndexAndSubjectIndex).then(function () {
                if (
                    ...
                    expect.fail({
                        diff: function (output, diff, inspect, equal) {
                            output.inline = true;
                            var prefixOutput = expect.subjectType.prefix(output.clone(), subject);
                            var suffixOutput = expect.subjectType.suffix(output.clone(), subject);
